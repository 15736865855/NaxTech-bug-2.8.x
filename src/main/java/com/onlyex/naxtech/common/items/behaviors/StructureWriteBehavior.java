...
public class StructureWriteBehavior implements IItemBehaviour, ItemUIFactory {
    ...
    private void exportLog(PlayerInventoryHolder playerInventoryHolder) {
        if (getPos(playerInventoryHolder.getCurrentItem()) != null && !playerInventoryHolder.player.world.isRemote) {
            ...
            JsonBlockPattern blockPattern = null;
            if (blockPos != null) {
                blockPattern = new JsonBlockPattern(playerInventoryHolder.player.world, blockPos[0].getX(), blockPos[0].getY(), blockPos[0].getZ(), blockPos[1].getX(), blockPos[1].getY(), blockPos[1].getZ());
            }
            ...
            NTLog.logger.info(builder.toString());
        }
    }
