...
import com.onlyex.naxtech.api.recipe.properties.AltitudeProperty;
...
public class NTMetaTileEntityCosmicRayDetector extends RecipeMapMultiblockController {
    private BlockPos topBlockPos = new BlockPos(0, -64, 0);

    public NTMetaTileEntityCosmicRayDetector(ResourceLocation metaTileEntityId) {
        super(metaTileEntityId, NTRecipeMaps.COSMIC_RAY_DETECTOR_RECIPES);
        this.recipeMapWorkable = new CosmicRayDetectorLogic(this);
    }

    @Override
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity iGregTechTileEntity) {
        return new NTMetaTileEntityCosmicRayDetector(metaTileEntityId);
    }
    ...
    protected TraceabilityPredicate altitudePredicate(IBlockState... allowedStates) {
        ...
        }, NTTraceabilityPredicate.getCandidates(allowedStates));
    }
    ...
    private static IBlockState getCasingAState() {
        return NTMetaBlocks.MACHINE_CASING.getState(NTBlockMachinelCasing.CasingType.QUANTUM_CASING);
    }
    private static IBlockState getCasingBState() {
        return NTMetaBlocks.MACHINE_CASING.getState(NTBlockMachinelCasing.CasingType.REFLECTIVE_CASING);
    }
    ...
    private static IBlockState getFrameState() {
        return MetaBlocks.FRAMES.get(NTMaterials.MARM200Steel).getBlock(NTMaterials.MARM200Steel);//BlackTitanium TODO
    }
    private static IBlockState getSensorState() {
        return NTMetaBlocks.GLASS_CASING.getState(NTBlockGlassCasing.CasingType.IR_BORON_SILICATE_GLASS_BLOCK);
    }

    @SideOnly(Side.CLIENT)
    @Override
    public ICubeRenderer getBaseTexture(IMultiblockPart iMultiblockPart) {
        return NTTextures.QUANTUM_CASING;
    }
    ...
    protected void addDisplayText(List<ITextComponent> textList) {
        ...
        if (this.isStructureFormed()) {
            textList.add(new TextComponentTranslation("naxtech.machine.cosmic_ray_detector.altitude", topBlockPos.getY()));
            textList.add(new TextComponentTranslation("naxtech.machine.cosmic_ray_detector.can_see_sky." + canSeeSky()));
        }
    }
    ...
    public void addInformation(ItemStack stack, @Nullable World world, @Nonnull List<String> tooltip, boolean advanced) {
        ...
        tooltip.add(I18n.format("naxtech.machine.cosmic_ray_detector.tooltip.1"));
        if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
            tooltip.add(I18n.format("naxtech.machine.cosmic_ray_detector.tooltip_shift.1"));
            tooltip.add(I18n.format("naxtech.machine.cosmic_ray_detector.tooltip_shift.2"));
        }else {
