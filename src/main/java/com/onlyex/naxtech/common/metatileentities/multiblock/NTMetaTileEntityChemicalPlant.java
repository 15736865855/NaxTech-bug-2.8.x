...
import com.onlyex.naxtech.loaders.formula.CatalystFormula;
...
public class NTMetaTileEntityChemicalPlant extends RecipeMapMultiblockController {
    ...
    public NTMetaTileEntityChemicalPlant(ResourceLocation metaTileEntityId) {
        super(metaTileEntityId, NTRecipeMaps.CHEMICAL_PLANT_RECIPES);
    ...
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity iGregTechTileEntity) {
        return new NTMetaTileEntityChemicalPlant(metaTileEntityId);
    }
    ...
    private void initMap() {
        ...
        List<IBlockState> listCasing = NTAPI.MAP_CP_CASING.entrySet().stream()
                .sorted(Comparator.comparingInt(entry -> ((WrappedIntTier) entry.getValue()).getIntTier()))
                .map(Map.Entry::getKey)
                .collect(Collectors.toList());
        List<IBlockState> listTube = NTAPI.MAP_CP_TUBE.entrySet().stream()
                .sorted(Comparator.comparingInt(entry -> ((WrappedIntTier) entry.getValue()).getIntTier()))
                .map(Map.Entry::getKey)
                .collect(Collectors.toList());
        List<IBlockState> listMachineCasing = NTAPI.MAP_MACHINE_CASING.entrySet().stream()
                .sorted(Comparator.comparingInt(entry -> ((WrappedIntTier) entry.getValue()).getIntTier()))
                .map(Map.Entry::getKey)
                .collect(Collectors.toList());
        ...
        finalListCoil = NTUniverUtil.consistentList(listCoil, maxLeng);
        finalListCasing = NTUniverUtil.consistentList(listCasing, maxLeng);
        finalListTube = NTUniverUtil.consistentList(listTube, maxLeng);
        finalListMachineCasing = NTUniverUtil.consistentList(listMachineCasing, maxLeng);
    ...
    protected void formStructure(PatternMatchContext context) {
        ...
        this.coilLevel = NTUniverUtil.getOrDefault(() -> coilType instanceof IHeatingCoilBlockStats,
        ...
        this.casingTier = NTUniverUtil.getOrDefault(() -> casingTier instanceof WrappedIntTier,
                ...
                0);
        this.tubeTier = NTUniverUtil.getOrDefault(() -> tubeTier instanceof WrappedIntTier,
                () -> ((WrappedIntTier)tubeTier).getIntTier(),
                0);
        this.voltageTier = NTUniverUtil.getOrDefault(() -> voltageTier instanceof WrappedIntTier,
                () -> ((WrappedIntTier)voltageTier).getIntTier(),
                0);

        this.tier = Math.min(this.casingTier,this.tubeTier);

        this.writeCustomData(NTDataCode.NT_CHANNEL_3, buf -> buf.writeInt(this.casingTier));
    }
    ...
    protected void initializeAbilities() {
        ...
        inputItem.addAll(this.getAbilities(NTMultiblockAbility.CATALYST_MULTIBLOCK_ABILITY));
    ...
    public void update() {
        ...
        if(this.getWorld().isRemote && this.casingTier == 0){
            this.writeCustomData(NTDataCode.NT_CHANNEL_2, buf -> {});
        }
    }
    ...
    protected BlockPattern createStructurePattern() {
        return FactoryBlockPattern.start()
                ...
                .where('E', NTTraceabilityPredicate.NT_CP_CASING.get()
                        ...
                        .or(abilities(NTMultiblockAbility.CATALYST_MULTIBLOCK_ABILITY).setMaxGlobalLimited(2).setPreviewCount(1))
                        .or(abilities(MultiblockAbility.INPUT_ENERGY).setMinGlobalLimited(1).setMaxGlobalLimited(2).setPreviewCount(1)))
                .where('C', NTTraceabilityPredicate.NT_CP_CASING.get())
                .where('X', heatingCoils())
                .where('M', NTTraceabilityPredicate.NT_MACHINE_CASINGS.get())
                .where('T', NTTraceabilityPredicate.NT_CP_TUBE.get())
                .where('#', any())
                .where('A',air())
                .build();
    }
    ...
    public List<MultiblockShapeInfo> getMatchingShapes() {
        ...
        if (Blocks.AIR != null) {
            builder = MultiblockShapeInfo.builder()
                    ...
                    .where('S', NTMetaTileEntities.CHEMICAL_PLANT, EnumFacing.SOUTH)
                    ...
                    .where('Z', NTMetaTileEntities.MULTIPART_CATALYST_HATCH, EnumFacing.SOUTH)
    ...
    public void receiveCustomData(int dataId, PacketBuffer buf) {
        ...
        if(dataId == NTDataCode.NT_CHANNEL_3){
            this.casingTier = buf.readInt();
        }
        if(dataId == NTDataCode.NT_CHANNEL_2){
            this.writeCustomData(NTDataCode.NT_CHANNEL_3, buf1 -> buf1.writeInt(this.casingTier));
        }
    }
    ...
    protected class ChemicalPlantLogic extends MultiblockRecipeLogic {
        ...
        public boolean checkRecipe(@Nonnull Recipe recipe) {
            if (super.checkRecipe(recipe) && (recipe.getProperty(CasingTierProperty.getInstance(), 0) <= tier)) {
                List<ItemStack> list = getAbilities(NTMultiblockAbility.CATALYST_MULTIBLOCK_ABILITY).stream()
