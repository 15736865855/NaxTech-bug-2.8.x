...
import com.onlyex.naxtech.common.block.blocks.NTBlockPackagingline;
...
public class NTMetaTileEntityPackagingLine extends RecipeMapMultiblockController {

    public NTMetaTileEntityPackagingLine(ResourceLocation metaTileEntityId) {
        super(metaTileEntityId, NTRecipeMaps.PACKAGING_LINE_RECIPES);//8.30%
    }

    @Override
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity iGregTechTileEntity) {
        return new NTMetaTileEntityPackagingLine(metaTileEntityId);
    }
    ...
    protected BlockPattern createStructurePattern() {
        return FactoryBlockPattern.start()
                ...
                .where('C',  states(NTMetaBlocks.PACKAGING_LINE.getState(NTBlockPackagingline.MultiblockCasingType.PACKAGING_LINE_CASING)))
    ...
    protected static IBlockState getMCasingState() {
        return NTMetaBlocks.PACKAGING_LINE.getState(NTBlockPackagingline.MultiblockCasingType.PACKAGING_LINE_MACHINE_CASING);
    }

    @Nonnull
    protected static IBlockState getCCasingState() {
        return NTMetaBlocks.PACKAGING_LINE.getState(NTBlockPackagingline.MultiblockCasingType.PACKAGING_LINE_CONTROL_CASING);
    }

    @Nonnull
    protected static IBlockState getBWGlasState() {
        return NTMetaBlocks.GLASS_CASING.getState(NTBlockGlassCasing.CasingType.TI_BORON_SILICATE_GLASS_BLOCK);
    }
    @Nonnull
