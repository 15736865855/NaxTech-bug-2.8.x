...
public class NTMetaTileEntityGeneralProcessingPlant extends MultiMapMultiblockController implements addMaintenanceText {
...
//    private boolean storedTaped = false;
    public NTMetaTileEntityGeneralProcessingPlant(ResourceLocation metaTileEntityId) {
        super(metaTileEntityId, new RecipeMap[]{
                NTRecipeMaps.GENERAL_RECIPES_A,
                NTRecipeMaps.GENERAL_RECIPES_B,
                NTRecipeMaps.GENERAL_RECIPES_C
        });
    ...
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity iGregTechTileEntity) {
        return new NTMetaTileEntityGeneralProcessingPlant(metaTileEntityId);
    }
    ...
    protected void formStructure(PatternMatchContext context) {
        ...
        if (this.hasMaintenanceMechanics() && ConfigHolder.machines.enableMaintenance) { // nothing extra if no maintenance
            if (getAbilities(NTMultiblockAbility.INDUSTRIAL_MAINTENANCE_MULTIBLOCK_ABILITY).isEmpty())
                return;
            maintenanceHatch = getAbilities(NTMultiblockAbility.INDUSTRIAL_MAINTENANCE_MULTIBLOCK_ABILITY).get(0);
    ...
    protected BlockPattern createStructurePattern() {
        return FactoryBlockPattern.start()
                ...
                .where('M', abilities(NTMultiblockAbility.INDUSTRIAL_MAINTENANCE_MULTIBLOCK_ABILITY))
    ...
    private static IBlockState getCasingAState() {
        return null;//NTMetaBlocks.EP_MULTIBLOCK_CASING_B.getState(EPBlockMultiblockCasingB.CasingType.MARAGING_STEEL_CASING);todo
    }

    private static IBlockState getCasingBState() {
        return null;//NTMetaBlocks.EP_MULTIBLOCK_CASING_B.getState(EPBlockMultiblockCasingB.CasingType.GENERAL_PROCESSING_CASING);
    }
    ...
    private static IBlockState getFrameState() {
        return MetaBlocks.FRAMES.get(NTMaterials.MaragingSteel250).getBlock(NTMaterials.MaragingSteel250);
    }
    ...
    public ICubeRenderer getBaseTexture(IMultiblockPart iMultiblockPart) {
        return NTTextures.MARAGING_STEEL_CASING;
    }
    ...
    public void addInformation(ItemStack stack, @Nullable World player, List<String> tooltip, boolean advanced) {
        ...
        tooltip.add(I18n.format("naxtech.multiblock.general_processing_plant.tooltip.1"));
        tooltip.add(I18n.format("naxtech.multiblock.general_processing_plant.tooltip.2"));
        tooltip.add(I18n.format("naxtech.multiblock.general_processing_plant.tooltip.3"));
        if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
            tooltip.add(I18n.format("naxtech.multiblock.general_processing_plant.shift_tooltip.1"));
            tooltip.add(I18n.format("naxtech.multiblock.general_processing_plant.shift_tooltip.2"));
            tooltip.add(I18n.format("naxtech.multiblock.general_processing_plant.shift_tooltip.3"));
            tooltip.add(I18n.format("naxtech.multiblock.general_processing_plant.shift_tooltip.4"));
        } else {
    ...
    public void causeMaintenanceProblems() {
        this.maintenance_problems &= (byte) ~(1 << ((int) (GTValues.RNG.nextFloat() * 6)));
    ...
    @Override
    public void addMaintenanceText(List<ITextComponent> textList) {
        if (hasMaintenanceProblems()) {
            ...
            if (((this.maintenance_problems >> 6) & 1) == 0)
                hoverEventTranslation.appendSibling(new TextComponentTranslation("naxtech.multiblock.universal.problem.soldering_iron", "\n"));

            textList.add(hoverEventTranslation.setStyle(new Style().setColor(TextFormatting.RED)));
        }
    }

    protected static class GeneralRecipeLogic extends MultiblockRecipeLogic {
