...
public class NTMetaTileEntityIndustrialPrimitiveBlastFurnace extends NoEnergyMultiblockController {

    private byte auxiliaryBlastFurnaceNumber = 0;

    private static final TraceabilityPredicate IS_SNOW_LAYER = new TraceabilityPredicate(bws -> GTUtility.isBlockSnow(bws.getBlockState()));//TODO

    public NTMetaTileEntityIndustrialPrimitiveBlastFurnace(ResourceLocation metaTileEntityId) {
        super(metaTileEntityId, RecipeMaps.PRIMITIVE_BLAST_FURNACE_RECIPES);
    }

    @Override
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity iGregTechTileEntity) {
        return new NTMetaTileEntityIndustrialPrimitiveBlastFurnace(metaTileEntityId);
    }
    ...
    protected BlockPattern createStructurePattern() {
        return FactoryBlockPattern.start()
                ...
                .where('A', NTTraceabilityPredicate.optionalStates("AuxiliaryBlastFurnace1", getFireBoxState()))
                .where('C', states(getFrameState()))
                .where('D', states(getCasingState()))
                .where('F', NTTraceabilityPredicate.optionalStates("AuxiliaryBlastFurnace2", getFireBoxState()))
                .where('G', NTTraceabilityPredicate.optionalStates("AuxiliaryBlastFurnace1", getCasingState()))
                .where('H', NTTraceabilityPredicate.optionalStates("AuxiliaryBlastFurnace1", getBoilerState()))
                .where('I', NTTraceabilityPredicate.optionalStates("AuxiliaryBlastFurnace2", getBoilerState()))
                .where('J', NTTraceabilityPredicate.optionalStates("AuxiliaryBlastFurnace2", getCasingState()))
    ...
    public List<MultiblockShapeInfo> getMatchingShapes() {
        ...
        if (Blocks.AIR != null) {
            builder = MultiblockShapeInfo.builder()
                    ...
                    .aisle("    CDDDC    ", "    CDSDC    ", "    CDDDC    ", "     DDD     ", "             ", "             ", "             ", "             ", "             ")
                    .aisle("     DDD     ", "             ", "             ", "             ", "             ", "             ", "             ", "             ", "             ")
                    .where('S', NTMetaTileEntities.INDUSTRIAL_PRIMITIVE_BLAST_FURNACE, EnumFacing.SOUTH)
