...
import com.onlyex.naxtech.api.recipe.properties.PHChangeProperty;
import com.onlyex.naxtech.api.recipe.properties.PHErrorRangeProperty;
import com.onlyex.naxtech.api.recipe.properties.PHProperty;
import com.onlyex.naxtech.api.utils.NTMathUtil;
...
public class NTMetaTileEntityFermentationTank extends RecipeMapMultiblockController implements IPHValue {
    private double pH = 7;

    public NTMetaTileEntityFermentationTank(ResourceLocation metaTileEntityId) {
        super(metaTileEntityId, NTRecipeMaps.FERMENTATION_TANK_RECIPES);
        this.recipeMapWorkable = new PHRecipeLogic(this);
    }

    @Override
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity metaTileEntityHolder) {
        return new NTMetaTileEntityFermentationTank(metaTileEntityId);
    }
    ...
    protected BlockPattern createStructurePattern() {
        return FactoryBlockPattern.start()
                ...
                .where('X', states(getCasingState()).setMinGlobalLimited(42).or(autoAbilities()).or(abilities(NTMultiblockAbility.BUFFER_MULTIBLOCK_ABILITY).setExactLimit(1)))
    ...
    protected void addDisplayText(List<ITextComponent> textList) {
        ...
        if (isStructureFormed())
            textList.add(new TextComponentTranslation("naxtech.machine.fermentation_tank.ph", String.format("%, .2f", this.pH)));
    }

    @Override
    public String[] getDescription() {
        List<String> list = new ArrayList<>();
        list.add(I18n.format("naxtech.machine.fermentation_tank.description"));
        Collections.addAll(list, LocalizationUtils.formatLines("naxtech.machine.fermentation_tank.extra1"));
        Collections.addAll(list, LocalizationUtils.formatLines("naxtech.machine.fermentation_tank.extra2"));
    ...
    public void changeCurrentPHValue(double ph_change) {
        this.pH = NTMathUtil.clamp(pH + ph_change, 0, 14);
        this.markDirty();
    }
