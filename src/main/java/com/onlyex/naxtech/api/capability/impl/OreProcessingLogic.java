...
public class OreProcessingLogic implements IWorkable{
        ...
        for (String name : OreDictionary.getOreNames()) {
            ...
            if (name.startsWith("crushedPurified")) {
                for (ItemStack stack : OreDictionary.getOres(name)) {
                    isCrushedPureOre.add(NTUniverUtil.stackToInt(stack));
                }
            } else if (name.startsWith("crushedCentrifuged")) {
                for (ItemStack stack : OreDictionary.getOres(name)) {
                    isThermal.add(NTUniverUtil.stackToInt(stack));
                }
            } else if (name.startsWith("crushed")) {
                for (ItemStack stack : OreDictionary.getOres(name)) {
                    isCrushedOre.add(NTUniverUtil.stackToInt(stack));
                }
            } else if (name.startsWith("dustImpure")) {
                for (ItemStack stack : OreDictionary.getOres(name)) {
                    isImpureDust.add(NTUniverUtil.stackToInt(stack));
                }
            } else if (name.startsWith("dustPure")) {
                for (ItemStack stack : OreDictionary.getOres(name)) {
                    isPureDust.add(NTUniverUtil.stackToInt(stack));
                }
            } else if (name.startsWith("ore")) {
                for (ItemStack stack : OreDictionary.getOres(name)) {
                    isOre.add(NTUniverUtil.stackToInt(stack));
                }
            }
        }
    }
    ...
    private void doMac(HashSet<Integer>... tables) {
        ...
        if (midProduct != null) {
            for (ItemStack stack : midProduct) {
                int t_id = NTUniverUtil.stackToInt(stack);
    ...
    private void recipeProcessing() {
        ...
        for (ItemStack ore : tInput) {
            if (tCharged <= 0) break;
            int t_id = NTUniverUtil.stackToInt(ore);
            if (t_id == 0) continue;
    ...
    private void doCompress(List<ItemStack> list) {
        ...
        for (Integer id : r_product.keySet()) {
            ItemStack stack = NTUniverUtil.intToStack(id);
            midProduct[cnt] = NTUniverUtil.copyAmountUnsafe(r_product.get(id), stack);
            cnt++;
        }
    }
