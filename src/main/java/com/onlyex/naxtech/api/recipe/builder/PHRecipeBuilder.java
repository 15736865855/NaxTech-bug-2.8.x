package com.onlyex.naxtech.api.recipe.builder;
...
public class PHRecipeBuilder extends RecipeBuilder<PHRecipeBuilder> {
    ...
    public PHRecipeBuilder pH(double ph) {
        if (ph < 0 || ph > 14) {
            NTLog.logger.error("The pH value must be between 0 and 14.", new IllegalArgumentException());
            ...
            if (i > 2) {
                NTLog.logger.error("The maximum number of decimal places for pH is two.", new IllegalArgumentException());
                this.recipeStatus = EnumValidationResult.INVALID;
            }
        }
    ...
    public PHRecipeBuilder pHChange(double ph_change) {
        if (ph_change < -14 || ph_change > 14) {
            NTLog.logger.error("The pH change value must be between -14 and 14.", new IllegalArgumentException());
            this.recipeStatus = EnumValidationResult.INVALID;
        } else {
            int i = 0;
            do {
            ...
            if (i > 2) {
                NTLog.logger.error("The maximum number of decimal places for pH change is two.", new IllegalArgumentException());
                this.recipeStatus = EnumValidationResult.INVALID;
            }
        }
    ...
    public PHRecipeBuilder pHErrorRange(double ph_error_range) {
        if (ph_error_range < 0 || ph_error_range > 14) {
            NTLog.logger.error("The pH error range value must be between 0 and 14.", new IllegalArgumentException());
            this.recipeStatus = EnumValidationResult.INVALID;
        } else {
            int i = 0;
            do {
            ...
            if (i > 2) {
                NTLog.logger.error("The maximum number of decimal places for pH error range is two.", new IllegalArgumentException());
                this.recipeStatus = EnumValidationResult.INVALID;
            }
        }
